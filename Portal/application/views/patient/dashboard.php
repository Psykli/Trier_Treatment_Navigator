...
<div class="row">
	...
	<div class="col-sm-2">
		<!-- ÜBUNGSPORTAL DERZEIT INAKTIV -->
		<!--
		<a href="<?php echo base_url(); ?>index.php/patient/patient/index/exercises/" type="button" id="ex_button" class="btn btn-default btn-lg <?php echo $questionnaireAvailable ? 'disabled':'';?>" style="width:170px;">
			<span class="glyphicon glyphicon-list-alt" style="font-size: 36px; color: #31708f"></span><br />
			<br />Zu den Übungen
		</a>
		<?php if($questionnaireAvailable): ?>
			<span id="quest_info" class="label label-warning">Bitte Fragebögen ausfüllen!</span>
		<?php endif; ?>	
		-->	
	</div>
	...
	<?php //if( strcasecmp( $username, '9999P99' ) == 0 || strcasecmp( $username, '2310P11' ) == 0 || strcasecmp( $username, '2250P11' ) == 0): ?>
	...
	<div class="col-sm-6">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">Erinnerungen</h3>
			</div>	
			<div class="panel-body">
				<?php if( !isset( $remindsPlaner ) OR ( is_null( $remindsPlaner[0] ) && is_null( $remindsPlaner[1] ) && is_null( $remindsPlaner[2] ) ) ): ?>
					<div class="alert alert-info" role="alert">
						Sie haben zur Zeit keine Erinnerungen
					</div>
				<?php else: ?>
					<p>
						Hier sehen Sie die geplante Aktivitäten für heute und die kommenden zwei Tage. Bei einem Klick auf eine Aktivtiät erhalten Sie
						detailierte Informationen zur Aktivität.
					</p>
					<table class="table">
						<tr>
							<th colspan="2">Heute <?php echo ( isset( $remindsPlaner[0] ) ) ? ' - '.date( 'd.m.Y', strtotime($remindsPlaner[0][0]->tag ) ) : ' - keine'; ?></th>
						</tr>
						<?php foreach( $remindsPlaner[0] as $heute ): ?>
							<tr>
								<td><?php echo date( 'H:i', strtotime( $heute->uhrzeit ) ); ?></td>
								<td><?php echo anchor( 'patient/ex_aktiv/index/schedule', $heute->aktivitaet ); ?></td>
							</tr>
						<?php endforeach; ?>
						
						<tr>
							<th colspan="2">Morgen <?php echo ( isset( $remindsPlaner[1] ) ) ? ' - '.date( 'd.m.Y', strtotime($remindsPlaner[1][0]->tag ) ) : ' - keine'; ?></th>
						</tr>
						<?php foreach( $remindsPlaner[1] as $morgen ): ?>
							<tr>
								<td><?php echo date( 'H:i', strtotime( $morgen->uhrzeit ) ); ?></td>
								<td><?php echo anchor( 'patient/ex_aktiv/index/schedule', $morgen->aktivitaet ); ?></td>
							</tr>
						<?php endforeach; ?>
						
						<tr>
							<th colspan="2">Übermorgen <?php echo ( isset( $remindsPlaner[2] ) ) ? ' - '.date( 'd.m.Y', strtotime($remindsPlaner[2][0]->tag ) ) : ' - keine'; ?></th>
						</tr>
						<?php foreach( $remindsPlaner[2] as $uebermorgen ): ?>
							<tr>
								<td><?php echo date( 'H:i', strtotime( $uebermorgen->uhrzeit ) ); ?></td>
								<td><?php echo anchor( 'patient/ex_aktiv/index/schedule', $uebermorgen->aktivitaet ); ?></td>
							</tr>
						<?php endforeach; ?>
					</table>
				<?php endif; ?>

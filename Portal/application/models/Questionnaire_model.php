...
class Questionnaire_model extends CI_Model
{
    ...
    //MUSS UMGESCHRIEBEN WERDEN SOBALD INFORMATIONEN VERFÜGBAR SIND !!!!!!!!!!!!!
    private function _get_process_data( $username, $patientcode, $name, $list_pos )
    {
        $data = NULL;
        //get the 1st databasetablename
        $psychoeq_table = $this -> process_questionnaires -> get( $list_pos ) -> get_psychoeq_table( );
        //get the 2nd databasetablename (some process bows have two), otherwise
        // NULL
        $psychoeq_table2 = $this -> process_questionnaires -> get( $list_pos ) -> get_psychoeq_process_table( );
        //get the 1st db_data
        $db_result = $this -> Patient_model -> get_process_data( $username, $patientcode, $psychoeq_table );
        //get the 2nd db_data
        $db_result2 = $this -> Patient_model -> get_process_data( $username, $patientcode, $psychoeq_table2 );
		//var_dump($db_result);
        ...
        if( !is_null( $tmp ) )
        {
			...
			//Der IIP-12 wird nicht angezeigt, wenn es beispielsweise mehr als eine Sitzugn gibt, aber nur einen Mittelwert
			//Dann kann kein "Verlauf" gezeichnet werden, daher wird der IIP-12 nur angezeigt, wenn entsprechend viele Werte vorhanden sind
			$showIIP = TRUE;
			$counter = 0;
			...
			if( ( $tmp['name'] == 'iip-12' ) && ( count( $tmp['instances'] ) >= 2 ) && ( $counter < 2 ) )
				$showIIP = FALSE;

			if( ( ($tmp['name'] == 'gas' OR $tmp['name'] == 'tsb-mk' OR $tmp['name'] == 'tsb-pa' OR $tmp['name'] == 'tsb-pb' OR $tmp['name'] == 'tsb-ra' OR $tmp['name'] == 'tsb-tb' OR strpos($tmp['name'],'tsb-ee') !== FALSE OR $tmp['name'] == 'tsb-wb' OR $tmp['name'] == 'tsb-pbmo' ) &&
					floor( count( $tmp['instances'], COUNT_RECURSIVE ) / count( $tmp['instances'] ) ) > 2 ) OR ( $tmp['name'] == 'iip-12' && $showIIP) )
				$graph = array( 'graph' => $this -> jpgraph -> process_graph( $username, $name, $tmp['name'], $tmp['title'], $tmp ) );
			else{
                $tmp = $this -> remove_ot($tmp);
                if ( $tmp['name'] != 'tsb-mk' && $tmp['name'] != 'tsb-pa' && $tmp['name'] != 'tsb-pb' && $tmp['name'] != 'tsb-ra' && $tmp['name'] != 'tsb-tb' && $tmp['name'] != 'iip-12' && count($tmp['instances']) >= 2 ){
                    $tmp['view_status'] = $this->Patient_model->get_view_status( $patientcode );
                    $graph = array( 'graph' => $this -> jpgraph -> process_graph( $username, $name, $tmp['name'], $tmp['title'], $tmp ) );
                }
            }
            if( !is_null( $tmp ) AND !empty($graph['graph']) )
            {
                $tables = array();
                $tables['table1'] = $psychoeq_table;
                $tables['table2'] = $psychoeq_table2;
                $data = array_merge( $tmp, $graph,$tables );
                log_message( 'debug', "ADD process data: " . $tmp['name'] );
            }//if
        }//if
        else
        {
            // error_log( "[OBJ]->name = " . $name . " IS NULL." );
        }//else
    ...
    */
    private function _fetch_process_data($username, $patientcode, $name, $list_pos)
    {
        ...
        $db_table = $this-> process_questionnaires -> get($list_pos) -> get_db_table();
        ...
        if ($name == 'hscl-11') {
            ...
            $db_table2 = $this->  process_questionnaires -> get($list_pos) -> get_db_process_table();
        }
    ...
    public function get_all_items(){
        ...
        if($query->num_rows() > 0){
            ...
            foreach ($result as $value) {
                if(!isset($items[$value->table_name])){
                    $items[$value->table_name] = array();
                }
                if(!isset($items[$value->table_name][$value->language])){
                    $items[$value->table_name][$value->language] = array();
                }
                $item_names = json_decode($value->item_names);
                $item_texts = json_decode($value->item_texts);
                $item_merge = array_combine($item_names,$item_texts);
                $items[$value->table_name][$value->language] = $item_merge;
            }
        }
        return $items;
    }

    public function get_values_with_info_of_all_items($language,$instance,$patientcode){
        $items = $this->get_all_items();

        $out = array();
        foreach($items as $table => $lang){
            if($this->db->table_exists(strtolower($table))){
                $this->db->select(implode(',',array_keys($lang[$language])));
                $this->db->from($table);
                $this->db->where('instance',$instance);
                $this->db->where('code',$patientcode);
                ...
                if($query->num_rows() > 0){
                    ...
                    $out[$table] = array_merge_recursive($lang[$language],$result[0]);
    ...
    public function get_high_items($items, $table){
        $this->db->select('min, max, item_invert');
        ...
        if($query->num_rows() > 0){
            ...
            foreach($result as $r){
                $tmp = (array)json_decode($r->item_invert);
                $invert = array_merge($invert, $tmp);
            }


            foreach($items as $item_name => $val){
                $is_inverted = array_key_exists($item_name,$invert);
                $high = $is_inverted ? intval($min) + 1 : intval($max) - 1;
                if(($is_inverted && $val[1] <= $high) || 
                (!$is_inverted && $val[1] >= $high)){
                    $high_items[$item_name] = $val;
                }
            }
        }
    ...
    public function get_process_scales_data($patientcode,$scale_name = null){
        $this->db->from('questionnaire_process_scales');
        if(isset($scale_name)){
            $this->db->where('name',$scale_name);
        }
        ...
        if($query->num_rows() > 0 ){
            ...
            foreach($result as $entry){
                ...
                $res = $this->get_result_of_items($items,$item_invert,$entry->table_name,$patientcode);
    ...
    public function get_process_scales_info_by_table($table){
        $this->db->from('questionnaire_process_scales');
        $this->db->where('table_name',$table);
    ...
    public function get_process_scales_info_by_scale($name){
        $this->db->from('questionnaire_process_scales');
        $this->db->where('name',$name);
    ...
    public function get_status_scales_data($patientcode){
        $this->db->from('questionnaire_status_scales');
        $this->db->order_by('name');
        ...
        if($query->num_rows() > 0 ){
            ...
            foreach($result as $entry){
                ...
                $mean = $entry->mean;
                $sd = $entry->sd;
                $res = $this->get_result_of_items($items,$item_invert,$entry->table_name,$patientcode, $mean, $sd);
                $means[$entry->table_name][$entry->name] = $res[$entry->table_name];
            }
        }
    ...
    public function get_status_scales_info($name){
        $this->db->from('questionnaire_status_scales');
    ...
    private function get_result_of_items($items,$item_invert,$table,$patientcode, $mean = null, $sd = null){
            ...
            foreach($result as $entry){
                ...
                foreach($entry as $key => $val){
                    if(array_key_exists($key,$item_invert)){
                        ...
                        $entry_list[] = floatval($val);
                    }
                }
                $result = $this->pf_math->mean($entry_list);
                if(!isset($results[$table])){
                    $results[$table] = array();
                }
                if(isset($mean)){
                    $p_result = $this->pf_math->pr_b($result,$mean,$sd);
                    $results[$table][$instance] = array($p_result,$result);
                } else {
                    $results[$table][$instance] = array($date => $result);
                }
            }
        }

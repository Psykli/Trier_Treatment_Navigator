(function ($) {
  ...
  DayScheduleSelector.prototype.renderHeader = function () {
    ...
    $.each(days, function (i, _) { html += '<th style="text-align:center;">' + (stringDays[i] || '') + '</th>'; });
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(generateDates(start, end, interval), function (i, d) {
      ...
      $el.append('<tr><td class="time-label" style="text-align:center;">' + hmmAmPm(d) + '</td>' + daysInARow + '</tr>');
    });
  };
   ...
   * @public
   */
  DayScheduleSelector.prototype.reset = function () {
    var plugin = this
  ...
  DayScheduleSelector.prototype.resetSingleDay = function (dayNumber) {
    var plugin = this
    if (dayNumber == 0){
      $.each(this.options.days, function (_, v) {
        plugin.$el.find(".time-slot[data-day='" + v + "']").each(function () {
